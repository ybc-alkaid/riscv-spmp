[[spmp]]
== "Sspmp/Sspmpsw/Smpmpdeleg" Extensions, Version 1.0
...
Writes to the `sspmpswitch` CSR are not ordered with respect to subsequent memory accesses.
To enforce ordering, software must execute an `SFENCE.VMA` instruction with `rs1=x0` and `rs2=x0`.
