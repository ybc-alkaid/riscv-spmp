...
. [#norm:ext:Sspmp:dep_sscsrind]#The `Sscsrind` extension for indirect CSR access must be implemented.#
+
. [#norm:ext:Sspmp:dep_sum_writable]#The `sstatus.SUM` (permit Supervisor User Memory access) bit must be *writable*.#
+
. [#norm:ext:Sspmp:dep_mxr_writable]#The `sstatus.MXR` (Make eXecutable Readable) bit must be *writable*.#
...
On RV32 systems, [#norm:csr:spmpaddr:format_rv32]#each `spmpaddr` register encodes a 34-bit physical address from bit 33 down to bit 2, as illustrated in <<spmpaddr-rv32>>.#
On RV64 systems, [#norm:csr:spmpaddr:format_rv64]#each `spmpaddr` register encodes a 56-bit physical address from bit 55 down to bit 2, as shown in <<spmpaddr-rv64>>.#
...
Refer to the {privspec}, Physical Memory Protection, Address Matching.
...
. [#norm:csrfld:spmpcfg:RWX:bits]#The R, W, and X bits govern permissions for read, write, and instruction execution, respectively.#
. [#norm:csrfld:spmpcfg:A:field]#The A field, which defines the address-matching mode, is detailed in <<address-matching>>.#
. Bits 5 and 6 are reserved for future standard use.
. [#norm:csrfld:spmpcfg:L:lock_bit]#The L (lock) bit designates an entry as locked.
Setting the L bit locks the SPMP entry, regardless of the A field's setting (even OFF).#
[#norm:csrfld:spmpcfg:L:write_ignored]#Attempts to write to locked `spmpcfg[i]` and `spmpaddr[i]` registers using the `siselect` CSR are ignored.#
[#norm:csrfld:spmpcfg:L:tor_prev_addr]#If a locked entry has `spmpcfg[i].A` set to TOR, writes to the preceding `spmpaddr[i-1]` via `siselect` are also ignored.#
. [#norm:csrfld:spmpcfg:U:mode_select]#For any rule not designated as a `Shared-Region`, the U bit determines if it is `U-mode` (when set) or `S-mode-only` (when clear), as explained in <<encoding>>.#
. [#norm:csrfld:spmpcfg:SHARED:shared_region]#The SHARED bit identifies a rule as a `Shared-Region` rule.#
...
[[norm:csrfld:spmpcfg:A:match_off]]
[[norm:csrfld:spmpcfg:A:match_tor]]
[[norm:csrfld:spmpcfg:A:match_na4]]
[[norm:csrfld:spmpcfg:A:match_napot]]
...
[#norm:csrfld:spmpcfg:A:tor_entry0]#Particularly, if `spmpcfg[0].A` is set to TOR, zero is used for the lower bound.#
...
. [#norm:ext:Sspmp:smode_rule_enforce]#An *S-mode-only* rule *enforces* accesses for Supervisor mode and *denies* accesses for User mode.#
+
. [#norm:ext:Sspmp:umode_rule_enforce]#A *U-mode* rule is always enforced for User mode accesses.# Its behavior for Supervisor mode accesses depends on the `sstatus.SUM` bit.
+
* [#norm:ext:Sspmp:umode_rule_sum_effect]#With `sstatus.SUM` set, the rule is enforced for Supervisor mode data accesses,# but [#norm:ext:Sspmp:umode_rule_enforceNoX]#execution permission is denied (termed *EnforceNoX* in <<spmpencode>>)#. This prevents the OS from executing the memory of an unprivileged process at all times.
+
...
+
. [#norm:ext:Sspmp:shared_rule_sum_ignored]#The encoding `spmpcfg.SHARED == 1 and spmpcfg.U == 1` defines a *Shared-Region* rule. For shared regions, the state of the `sstatus.SUM` bit is irrelevant.#
+
. [#norm:ext:Sspmp:shared_rule_enforce]#A *Shared-Region* rule is *enforced* for both Supervisor and User modes, but it imposes the constraint that read and write permissions for User mode are mutually exclusive.#
+
. The R, W, and X bits collectively form a WARL field.
+
. The encoding [#norm:csrfld:spmpcfg:RWX:rwx_010_reserved]#`spmpcfg.RWX=010`#, [#norm:csrfld:spmpcfg:RWX:rwx_011_reserved]#`spmpcfg.RWX=011`#, and the combination `spmpcfg.SHARED == 1 and spmpcfg.U == 0` are all reserved for future standard use.
...
[[spmpencode]]
image::SPMP_Encoding_Table_v12.svg[title="SPMP Encoding Table"]
...
+
. [#norm:ext:Sspmp:match_priority]#The lowest-numbered SPMP entry that matches any byte of an access (indicated by an address and the accessed length) determines whether that access is allowed or denied.#
+
...
+
. [#norm:ext:Sspmp:match_irrespective_perm_bits]#This matching is done irrespective of the SHARED, U, R, W, and X bits.#
+
. [#norm:ext:Sspmp:no_match_deny]#If the effective privilege mode of the access is S/U and no SPMP entry matches, but at least one SPMP entry is implemented, the access is `denied`.#
+
...
[[spmp-and-paging]]
=== SPMP and Paging
[#norm:ext:Sspmp:paging_mutual_exclusion]#SPMP and paged virtual memory are mutually exclusive.#
...
[#norm:csr:siselect:spmpaddr_mapping]#`sireg` is used to access the `spmpaddr` register,# while [#norm:csr:siselect:spmpcfg_mapping]#`sireg2` is used for the `spmpcfg` register.#
...
[#norm:csr:siselect:oob_read_zero]#An access to an out-of-bounds index via `siselect` will return zero on read# and [#norm:csr:siselect:oob_write_ignored]#be ignored on write.#
...
[[norm:ext:Sspmp:instr_page_fault]]
[[norm:ext:Sspmp:load_page_fault]]
[[norm:ext:Sspmp:store_page_fault]]
